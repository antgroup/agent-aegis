import type { ProbeEvent } from "./event.js";

/**
 * Bounded event queue with optional sampling — the probe→judge backpressure
 * layer (M10 P1.4).
 *
 * Probes produce events at kernel speed; judges consume at async speed.
 * Without a bound, a burst of syscalls can grow the in-memory queue without
 * limit. EventQueue caps the depth and, when full, drops new events and
 * reports the loss via an onDrop callback (wired to a drop-marker in the
 * JSONL log so no loss is silent).
 *
 * An optional sampleRate < 1.0 pre-emptively thins the stream under load
 * using a pid/timestamp hash so the same process's events tend to be kept or
 * dropped together (more useful for attribution than random thinning).
 */
export interface EventQueueOptions {
  /** Maximum queue depth (default 4096). Events beyond this are dropped. */
  maxDepth?: number;
  /**
   * Sampling rate 0..1 (default 1.0 = no sampling).
   * 0.1 = keep roughly 1 in 10 events when the queue is under pressure.
   * Sampling is applied **before** the depth check so that, even before the
   * queue fills, a configured sampleRate can reduce throughput.
   */
  sampleRate?: number;
  /** Called when events are dropped, for metrics / logging. */
  onDrop?: (count: number, reason: "queue_full" | "sampled") => void;
}

export type EventHandler = (event: ProbeEvent) => void | Promise<void>;

export class EventQueue {
  private readonly queue: ProbeEvent[] = [];
  private readonly maxDepth: number;
  private readonly sampleRate: number;
  private readonly onDrop: EventQueueOptions["onDrop"];
  private readonly handler: EventHandler;

  private _dropped = 0;
  private _sampled = 0;
  private _enqueued = 0;
  private _processed = 0;
  private processing = false;
  private readonly pendingHandlers = new Set<Promise<void>>();

  constructor(handler: EventHandler, opts: EventQueueOptions = {}) {
    this.handler = handler;
    this.maxDepth = opts.maxDepth ?? 4096;
    this.sampleRate = opts.sampleRate ?? 1.0;
    this.onDrop = opts.onDrop;
  }

  /**
   * Enqueue an event. Returns true if the event was accepted (queued or
   * directly processed), false if it was dropped.
   *
   * Dropping can happen for two reasons:
   * 1. **sampled** — the sampleRate pre-emptively rejects the event.
   * 2. **queue_full** — the queue is at capacity.
   *
   * In both cases `onDrop` is called and the respective counter increments.
   */
  enqueue(event: ProbeEvent): boolean {
    // Pre-emptive sampling. Use a simple mixing function over the event id
    // for better distribution than pid/timestamp XOR (which clusters when
    // timestamps are close together, as they are in burst events).
    if (this.sampleRate < 1.0) {
      // FNV-1a-like hash of the event id string for uniform distribution.
      let h = 2166136261;
      for (let i = 0; i < event.id.length; i++) {
        h ^= event.id.charCodeAt(i);
        h = (h * 16777619) >>> 0;
      }
      if ((h & 0xffff) / 0xffff >= this.sampleRate) {
        this._sampled++;
        this.onDrop?.(1, "sampled");
        return false;
      }
    }

    // Queue-full drop.
    if (this.queue.length >= this.maxDepth) {
      this._dropped++;
      this.onDrop?.(1, "queue_full");
      return false;
    }

    this.queue.push(event);
    this._enqueued++;
    this.scheduleFlush();
    return true;
  }

  /** Current depth of the pending queue. */
  get size(): number {
    return this.queue.length;
  }

  /** Total events dropped because the queue was full. */
  get dropped(): number {
    return this._dropped;
  }

  /** Total events discarded by sampling. */
  get sampled(): number {
    return this._sampled;
  }

  /** Total events successfully enqueued (not counting sampled/dropped). */
  get enqueued(): number {
    return this._enqueued;
  }

  /** Total events processed (handler called). */
  get processed(): number {
    return this._processed;
  }

  /**
   * Reset the drop/sample counters (e.g. after writing a drop-marker so
   * the next marker only covers the delta).
   */
  resetCounters(): { dropped: number; sampled: number } {
    const d = this._dropped;
    const s = this._sampled;
    this._dropped = 0;
    this._sampled = 0;
    return { dropped: d, sampled: s };
  }

  /** Flush all pending events and wait for async handlers (for tests / shutdown). */
  async flush(): Promise<void> {
    while (this.queue.length > 0) {
      this.processOne(this.queue.shift()!);
    }
    await this.waitForHandlers();
  }

  private scheduleFlush(): void {
    if (this.processing) return;
    this.processing = true;
    // Use setImmediate (or Promise.resolve as fallback) to drain the queue
    // on the next event-loop tick. This ensures that a burst of enqueue()
    // calls batches into a single flush cycle.
    const scheduleNext = typeof (globalThis as Record<string, unknown>).setImmediate === "function"
      ? (globalThis as Record<string, unknown>).setImmediate as (cb: () => void) => unknown
      : (cb: () => void) => Promise.resolve().then(cb);
    scheduleNext(() => {
      this.drain();
    });
  }

  private drain(): void {
    // Drain up to 256 events per tick to avoid starving the event loop under
    // very high throughput. Remaining events are rescheduled.
    const batch = Math.min(this.queue.length, 256);
    for (let i = 0; i < batch; i++) {
      this.processOne(this.queue.shift()!);
    }
    if (this.queue.length > 0) {
      this.scheduleFlush();
    } else {
      this.processing = false;
    }
  }

  private processOne(event: ProbeEvent): void {
    this._processed++;
    try {
      const pending = Promise.resolve(this.handler(event))
        .catch(() => {
          // Swallow — the bus layer handles error reporting per subscriber.
        })
        .finally(() => {
          this.pendingHandlers.delete(pending);
        });
      this.pendingHandlers.add(pending);
    } catch {
      // Swallow — the bus layer handles error reporting per subscriber.
    }
  }

  private async waitForHandlers(): Promise<void> {
    while (this.pendingHandlers.size > 0) {
      await Promise.all([...this.pendingHandlers]);
    }
  }
}
