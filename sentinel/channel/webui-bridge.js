 ...
 */
export function appendWebuiDefenseEvent(eventsFile, event, verdict) {
    if (!verdict)
        return;
    const action = verdict.final.action;
    const result = action === "block" ? "blocked" : action === "observe" ? "observed" : null;
    if (!result)
        return; // allow → nothing to surface
    const record = {
...
function describeSyscall(event) {
    const a = event.args ?? {};
    ...
    if (event.syscall === "connect") {
        const dest = typeof a.dest === "string" ? a.dest : typeof a.addr === "string" ? a.addr : "";
