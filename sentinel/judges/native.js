 ...
 */
const DEFAULT_SCRATCH_DIR_PATTERNS = [
    /^\/tmp\//,
    /^\/var\/tmp\//,
    /^\/dev\/shm\//,
];
 ...
 */
function judgeSensitivePath(event, patterns) {
    const haystacks = [];
    if (event.syscall === "execve") {
        const argv = event.args.argv;
        if (Array.isArray(argv))
            haystacks.push(argv.map(String).join(" "));
        if (typeof event.args.path === "string")
            haystacks.push(event.args.path);
    }
 ...
 */
function judgeKernelEscape(event, scratchPatterns) {
    if (event.syscall !== "execve")
        return null;
 ...
 */
function judgeProcessTreeAnomaly(event, agentPids) {
    ...
    if (ppid === undefined)
        return null;
    if (agentPids.includes(ppid))
        return null;
    if (agentPids.includes(event.pid))
        return null;
    return {
        action: "observe",
        severity: "low",
