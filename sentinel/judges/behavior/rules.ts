import type { BufferedEvent } from "../../context/buffer.js";
import { isExternalAddr, SENSITIVE_PATTERNS } from "../../context/patterns.js";
import type { BehaviorRule, BehaviorRuleConfig, BehaviorRuleInput } from "./types.js";
...
export const builtInBehaviorRules: BehaviorRule[] = [
  {
    id: "causal-exfil",
    defaultConfig: {
      enabled: true,
    ...
    evaluate: ({ event, snapshot }) => {
      if (!hasCausalFlag(event, snapshot, "exfil")) return null;
      return {
  ...
  },
  {
    ...
    evaluate: ({ event, snapshot }) => {
      if (!hasCausalFlag(event, snapshot, "write_then_run")) return null;
      return {
  ...
  },
  {
    ...
    evaluate: ({ event, events, config }) => {
      ...
      if (sensitiveHits < minSensitive || externalConns < minExternal) return null;
      return {
  ...
  },
  {
    ...
    evaluate: ({ snapshot, config }) => {
      ...
      if (snapshot.features.processFanout <= max) return null;
      return {
  ...
  },
  {
    ...
    evaluate: ({ snapshot, config }) => {
      ...
      if (snapshot.features.externalConnCount <= max) return null;
      return {
  ...
  },
  {
    ...
    evaluate: ({ event, snapshot, config }) => {
      ...
      const matched = [currentPath, ...snapshot.binariesExecuted].filter(Boolean).find((path) =>
  ...
  },
  {
    ...
    evaluate: ({ event, events, config }) => {
      ...
      const match = window.find(
        (e) => e.syscall === "openat" && e.argsSummary && patterns.some((p) => p.test(e.argsSummary!)),
      );
  ...
  },
  {
    ...
    evaluate: ({ event, events, config }) => {
      ...
      for (const e of windowEvents(events, event.timestamp, readNumber(config, "windowMs", 60_000))) {
        if (e.syscall !== "connect" || !e.destAddr) continue;
        if (isInternalAddr(e.destAddr)) hosts.add(e.destAddr);
      }
      const min = readNumber(config, "minDistinctInternalHosts", 3);
      if (hosts.size < min) return null;
      return {
...
function hasCausalFlag(event: { meta?: Record<string, unknown> }, snapshot: { causalChains: string[] }, flag: string): boolean {
  const current = event.meta?.causalFlags;
  return (
    (Array.isArray(current) && current.some((v) => v === flag)) ||
    snapshot.causalChains.includes(flag)
  );
}

function windowEvents(events: BufferedEvent[], now: number, windowMs: number): BufferedEvent[] {
  const cutoff = now - windowMs;
  return events.filter((e) => e.timestamp >= cutoff && e.timestamp <= now);
}

function isSensitiveOpen(event: BufferedEvent): boolean {
  return (
...
function isExternalConnect(event: BufferedEvent): boolean {
...
function currentExecPath(event: { args?: Record<string, unknown>; proc?: { exe?: string } }): string | undefined {
  if (typeof event.proc?.exe === "string") return event.proc.exe;
  const argv = event.args?.argv;
  if (Array.isArray(argv) && typeof argv[0] === "string") return argv[0];
  if (typeof event.args?.path === "string") return event.args.path;
  return undefined;
}

function readNumber(config: BehaviorRuleConfig, key: string, fallback: number): number {
...
function readPatterns(config: BehaviorRuleConfig, key: string, fallback: string[]): RegExp[] {
  const direct = config[key];
