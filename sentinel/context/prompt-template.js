...
export class PromptTemplate {
    ...
    renderUser(event, snapshot) {
        ...
        sections.push(`- Attribution: agent=${snapshot.attributionCounts.agent} descendant=${snapshot.attributionCounts.descendant} external=${snapshot.attributionCounts.external} unknown=${snapshot.attributionCounts.unknown}`);
        // Threat indicators
        ...
        if (event.toolName)
            sections.push(`- Tool: ${event.toolName}`);
        const attr = event.meta?.attribution;
        if (attr)
            sections.push(`- Attribution: ${attr}`);
        const causal = event.meta?.causalFlags;
        if (causal?.length)
            sections.push(`- Causal flags: ${causal.join(", ")}`);
        if (event.correlationId)
            sections.push(`- Correlation: ${event.correlationId}`);
        // Summarized args
        if (event.syscall === "openat" && typeof event.args?.path === "string") {
            sections.push(`- Path: ${event.args.path}`);
        }
        else if (event.syscall === "connect") {
            const addr = event.args?.addr ?? event.net?.daddr;
            const port = event.args?.port ?? event.net?.dport;
            if (addr)
                sections.push(`- Address: ${addr}${port ? `:${port}` : ""}`);
        }
        else if (event.syscall === "execve") {
            if (Array.isArray(event.args?.argv)) {
                sections.push(`- Command: ${event.args.argv.map(String).join(" ")}`);
            }
        }
