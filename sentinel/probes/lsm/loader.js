...
import { parseLsmRunnerMessage } from "./messages.js";
import { detectLsmSupport } from "./platform.js";
import { encodePolicyMessage, PolicyTable } from "./policy.js";
const PROBE_ID = "lsm";
const EVENT_SOURCE = "lsm";
const HERE = path.dirname(fileURLToPath(import.meta.url));
const DEFAULT_RUNNER_BIN = path.join(HERE, "runner", "dist", "lsm-runner");
export function createLsmProbe(opts = {}) {
...
function wireStderr(stderr, deps) {
    stderr.on("data", (chunk) => {
        ...
        for (const line of text.split("\n")) {
            if (line.trim())
                deps.runtime.logger.warn(`[lsm.runner] ${line}`);
        }
    });
}
/** Exported for tests. */
