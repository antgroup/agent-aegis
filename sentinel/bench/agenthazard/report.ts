...
export async function writeReportFiles(report: DryRunReport, outDir: string): Promise<void> {
  ...
  await fs.writeFile(
    ...
    JSON.stringify(report.selected, null, 2) + "\n",
    "utf8",
  );
...
export function renderMarkdown(report: DryRunReport): string {
  ...
  if (report.options.jailbreakMethods?.length) {
    lines.push(
      `- Jailbreak method filter: ${report.options.jailbreakMethods.map((m) => `\`${m}\``).join(", ")}`,
    );
  }
  lines.push("");
  ...
  for (const category of Object.keys(report.summary.byCategory).sort()) {
    lines.push(
      `| ${category} | ${report.summary.byCategory[category] ?? 0} | ${report.summary.selectedByCategory[category] ?? 0} |`,
    );
  }
  lines.push("");
  ...
  for (const method of Object.keys(report.summary.byJailbreakMethod).sort()) {
    lines.push(
      `| ${method} | ${report.summary.byJailbreakMethod[method] ?? 0} | ${report.summary.selectedByJailbreakMethod[method] ?? 0} |`,
    );
  }
  lines.push("");
  ...
  lines.push("|---|---|---|---|---|");
  for (const mapping of Object.values(CATEGORY_MAPPINGS).sort((a, b) =>
    a.category.localeCompare(b.category),
  )) {
    lines.push(
      `| ${mapping.category} | ${joinCell(mapping.expectedSignals)} | ${joinCell(mapping.expectedSnapshotFields)} | ${joinCell(mapping.expectedCausalChains)} | ${joinCell(mapping.knownGaps)} |`,
    );
  }
  lines.push("");
  ...
  for (const item of report.selected) {
    lines.push(
      `| ${item.sampleRank} | ${item.id} | ${item.category} | ${item.jailbreak_method} | ${item.decomposed_query.length} | ${item.source || "-"} |`,
    );
  }
  lines.push("");
