...
import { AGENTHAZARD_CATEGORIES, AGENTHAZARD_JAILBREAK_METHODS, } from "./types.js";
const CATEGORY_SET = new Set(AGENTHAZARD_CATEGORIES);
const METHOD_SET = new Set(AGENTHAZARD_JAILBREAK_METHODS);
...
export function sampleDataset(dataset, opts) {
    ...
    const filtered = dataset.filter((item) => {
        if (categoryFilter && !categoryFilter.has(item.category))
            return false;
        if (methodFilter && !methodFilter.has(item.jailbreak_method))
            return false;
        return true;
    });
...
export function summarizeDataset(dataset, selected) {
    return {
...
function sampleCategory(items, perCategory, seed) {
    ...
    if (selected.length < perCategory) {
        ...
        const remainder = deterministicShuffle(items.filter((item) => !selectedIds.has(item.id)), seedFor(seed, category));
...
function parseInstance(raw, index) {
