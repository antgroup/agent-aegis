...
| `AEGIS_TOKEN` | 设置后，所有 API 请求必须携带 `x-aegis-token`（或 `Authorization: Bearer`、`?token=`）匹配该值 | 空（未启用 token） |
...
> **安全说明**：管理 API 默认绑定 `127.0.0.1`，CORS 仅放行本机白名单 Origin。未设置 `AEGIS_TOKEN` 时，后端启用 CSRF 防护——拒绝带有非白名单 `Origin` 头的浏览器跨域请求（如恶意网页访问 `127.0.0.1` 的请求），同源 UI 与本地命令行工具不受影响。需要更强鉴权或远程访问时，请设置 `AEGIS_TOKEN`。
