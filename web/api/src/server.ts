...
function resolveApiToken(configDir: string): { token: string; fromEnv: boolean } {
  ...
  try {
    ...
    console.error("[agent-aegis-web] could not persist token file:", err);
  }
...
export function createServer(options: ServerOptions) {
  ...
  // The token is never served to the page (see static serving below). The
  // operator copies it from here (or sets it via AEGIS_TOKEN) and enters it
  // into the UI once.
  if (fromEnv) {
    console.log(
      "[agent-aegis-web] API auth enabled (from AEGIS_TOKEN). Enter your token in the WebUI to access the panel.",
    );
  } else {
    console.log("[agent-aegis-web] API auth enabled. Save this token — you must enter it in the WebUI:");
    console.log(`\n    ${token}\n`);
  }
